...
レコード構造:
  Header(40B) + [Kumi(4)+Odds(6)+Ninki(3)] × N組 + Vote(11) + crlf(2)
  N = max 306 (18頭なら P(18,2)=306)
"""
...
class O4Parser:
    ...
    RECORD_TYPE = "O4"
    HEADER_SIZE = 40
    ENTRY_SIZE = 13  # Kumi(4) + Odds(6) + Ninki(3)
    ...
    def parse(self, data: bytes) -> Optional[List[Dict[str, str]]]:
        ...
        try:
            ...
            for i in range(num_entries):
                ...
                row = dict(header)
                row["Kumi"] = kumi
                row["Odds"] = self._odds(self.decode_field(data[pos + 4:pos + 10]))
                row["Ninki"] = self.decode_field(data[pos + 10:pos + 13])
                row["Vote"] = vote
                results.append(row)
